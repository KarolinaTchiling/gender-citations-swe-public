...
gender_api_key="Enter_your_key_here"
...
for(i in r){
  ...
  clean_name <- gsub("[^a-zA-Z]", "", clean_name)  #strips any non alpha chars that will crash the API request

  # Pull json data from gender-api
  json_file=paste0("https://gender-api.com/get?name=",clean_name,
                   "&key=", gender_api_key)
